...
export class ToolMenuSService {
  ...
  setButtonPositionsLeft() {
    ...
    if (this.btnCountGerade === true) {
      for (const ind of this.btnToShow) {
        if (ind === true) {
  ...
  setPositionLeft() {
    if (this.selectedShape != null) {
      this.toolMenuLeftPos =
        this.selectedShape.left + this.selectedShape.width / 2 - this.toolMenuWidth / 2;
  ...
  setButtonsToShow() {
    ...
    if (this.selectedShape.instanceOf() === 'dialog') {
      ...
      for (const i of this.btnIDsAtRechteck) {
        this.btnToShow[i] = true;
      }
    }
    if (this.selectedShape.instanceOf() === 'subKreisLeft') {
      for (const i of this.btnIDsAtKreisLeft) {
        this.btnCountGerade =
          this.btnIDsAtKreisLeft.length % 2 > 0 ? false : true;
        this.btnToShow[i] = true;
      }
    }
    if (this.selectedShape.instanceOf() === 'subKreisRight') {
      for (const i of this.btnIDsAtKreisRight) {
        this.btnCountGerade =
          this.btnIDsAtKreisRight.length % 2 > 0 ? false : true;
        this.btnToShow[i] = true;
      }
    }
    if (this.selectedShape.instanceOf() === 'startShape') {
      for (const i of this.btnIDsAtStartShape) {
        this.btnCountGerade =
          this.btnIDsAtStartShape.length % 2 > 0 ? false : true;
        this.btnToShow[i] = true;
      }
    }
  }
